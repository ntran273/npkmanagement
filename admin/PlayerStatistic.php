...
class PlayerStatistic
{
   ...
   function __construct($name="", $time="0:0", $points=0, $assists=0, $rebounds=0, $gameid = 0)
   {
   ...
   function toTSV()
   {
       return implode("\t", [$this->name(), $this->playingTime(), $this->pointsScored(), $this->assists(), $this->rebounds(), $this->gameid()]);
   }
   ...
   function fromTSV(string $tsvString)
   {
     ...
     list($name, $time, $points, $assists, $rebounds, $gameid) = explode("\t", $tsvString);
     $this->name($name);
     $this->playingTime($time);
     $this->pointsScored($points);
     $this->assists($assists);
